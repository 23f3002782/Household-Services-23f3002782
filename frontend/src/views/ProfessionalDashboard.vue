<template>
	<div class="container p-5">
		...
		<div v-if="$route.path === '/professional/dashboard'" class="pt-3">
			...
			<div v-if="onGoingRequests.length === 0">
				<h3 class="text-center">No bookings yet...</h3>
				<p class="text-center">Check out new requests</p>
				<div class="d-grid gap-2 d-md-flex justify-content-md-center">
					<button
						class="btn btn-outline-dark btn-sm"
						@click="$router.push('/professional/requests')"
					>
						Click here
					</button>
				</div>
			...
			<div v-else>
				...
				>
					...
					>
						...
						<tbody>
							<tr
								v-for="(request, index) in onGoingRequests"
								:key="index"
							>
								<td>{{ index + 1 }}</td>
		...
		<div v-if="$route.path === '/professional/requests'" class="pt-3">
			...
			>
				<table class="table text-center table-borderless align-middle">
					...
					<tbody>
						<tr
							v-for="(newRequest, index) in newRequests"
							:key="index"
						>
							<td>{{ index + 1 }}</td>
							...
							<td>
								{{
									newRequest.date_of_request.split(
										"00:00:00"
									)[0]
								}}
							</td>
...
<script setup>
	...
	const onGoingRequests = computed(() =>
		myRequests.value.filter(
			(req) =>
				req.status === "assigned" || req.status === "closed by customer"
		)
	);
